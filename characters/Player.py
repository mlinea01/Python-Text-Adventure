...
class Player:
    ...
    def choose_attack(self):
        ...
        if len(attacks_enabled) > 0 or len(self.character.items) > 0:
            server.print_text("Choose your attack: ")
            attack_num = 1
            chosen_attack_num = -1
            for attack in self.character.attacks:
                if attack.enabled:
                    server.print_text(str(attack_num) + ". " + attack.name + " - " + attack.desc)
                else:
                    server.print_text(str(attack_num) + ". " + attack.name + " - (DISABLED)")
                attack_num += 1

            for item in self.character.items:
                server.print_text(str(attack_num) + ". " + item.name)
                attack_num += 1
